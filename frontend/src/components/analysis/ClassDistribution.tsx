  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            >
              {chartData.map((entry, index) => (
                <Cell
                  key={`cell-${index}`}
                  fill={`url(#${GRADIENT_IDS[entry.label]})`}
                  style={{
                    cursor: 'pointer',
          ...
          {chartData.map((item) => (
            <div key={item.label} className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">
                {item.name.split(' ')[0]}: <span className="font-semibold text-gray-900 dark:text-white">{item.percentage}%</span>
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
            Распределение классов (круговая диаграмма)
          </h3>
        </div>
        <ResponsiveContainer width="100%" height={350}>
          <PieChart>
        ...
        <div className="mt-4 flex justify-center gap-6 flex-wrap">
          {chartData.map((item) => (
            <div key={item.label} className="flex items-center gap-2">
              ...
              />
              <span className="text-sm text-gray-600 dark:text-gray-400">
                {item.name.split(' ')[0]}: <span className="font-semibold text-gray-900 dark:text-white">{item.value.toLocaleString('ru-RU')}</span>
              </span>
            </div>
          ))}
        </div>
